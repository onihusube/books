...
　`swap`インターフェースは唯一つ`swap`という操作のためだけのものです。それはその名の通り、同じ型のオブジェクト同士あるいは互換性のある型のオブジェクト同士の間でその値を交換する操作です。`swap`を用いると代入演算子を例外安全に対応しつつシンプルに書くことができるようになるので、`swap`はコピーやムーブよりもより基本的な操作だと見る向きもあります。
