...
ラムダ式の場合は戻り値型としてデフォルトで`auto`指定が使用されています。これを変更するには、後置戻り値型指定で別のプレースホルダ指定を行います。
...
暗黙変換をするか否か、どの経路で行うかについては戻り値型が決定した後、すなわち戻り値型推論の後で（コンパイル時に）決定されます。戻り値型推論が行われている場合はあまり大きな暗黙変換は行われず、CV修飾や参照修飾の調整に留まるはずです。
...
（実際には戻り値最適化を考慮しなければならないため戻り値の初期化はこれほど単純ではありません。戻り値最適化は後半の章の、そして本書のメインテーマです）
...
auto f5(double d) -> std::pair<int, float> {
  if (std::isnan(d)) {
    return {10, 0.0}; // ok
  } else {
    return {10, d};   // ng
