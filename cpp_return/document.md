...
とはいえ、レジスタのサイズには限界があり（例えば`rax`レジスタは64bit）、戻り値返しに使用するレジスタは関数呼び出し前後で退避（コピー）が発生するためクラス型の場合はそのコンストラクタとデストラクタの呼び出しが必要になります。そのため、ある一定以上のサイズのクラス型及びトリビアルではないクラス型については、戻り値のための領域を別に用意しておいて専用のレジスタを通してそのアドレスをやりとりします。この別に用意される領域には通常スタック領域が使用され、これは関数を呼び出す側が用意します。
...
このとき重要なのは、コピー省略を無効化したときでも戻り値返しに関して関数側がやることは変わらず、呼び出し側から渡された戻り値専用の領域に戻り値を構築してリターンすることだけ、という点です。戻り値用領域が何を指しているのかや戻り値のコピーが起こるかどうかなどのことは関数リターン後の呼び出し側で起こることで、関数はコピー省略が起こるかどうかに関わらず、デフォルトの動作としてNRVO的な戻り値の返し方をしています。ABIが指定する戻り値の返し方はコピー省略の種類によらず変わらないため、このことはRVO/NRVOが行われるかによらず同様です。
...
関数の戻り値返しがABIでどのように行われているかを見てみると、NRVOの制約の源泉が見えてきます。
...
ABIでは、呼び出し側が用意した領域のアドレスを受け取ってその領域に対して戻り値を構築するわけで、NRVOが行われるとすると`c`を直接その領域に構築することになります。すると、その場合に`b`が`false`だと別のオブジェクトをその領域に上書きしなければならないため、`c`を一旦破棄してから`{20}`で再構築することになります。当然、これはC++コード上の動作とは異なります（デストラクタの呼び出しタイミングが違う）。つまりは、`return`文それぞれで異なるオブジェクトを返していると、渡された戻り値用の領域にどのオブジェクトを構築すればいいのかわからなくなるため、NRVOが行われづらくなります。
...
C++標準で許可されているNRVOの条件の一つとして、`return`文のオペランドの変数名がその関数の仮引数名ではないこと、というのがあります。

関数の引数をどのように渡すかということもABI（呼び出し規約）の一部であり、整数型等組み込み型やトリビアルなクラス型などはレジスタに配置されて渡されます。一定サイズを超えるか非ヒトリビアルなクラス型になると、スタックに配置されそのポインタが渡され、引数が多いと後ろの方の引数は特定のスタック上に配置され渡されます。この時、このスタックを掃除する（デストラクタ呼び出し等）責任を持つのが呼び出し側なのか呼び出された関数側なのかがABIによって異なっています。x64の主要なABIは全て呼び出し側がスタック清掃を行いますが、x86やIA-32のABIでは呼び出された側がそれを行うものもあり混在しています。

関数を呼び出す側がスタック清掃の責任を持つ場合、そのスタックを清掃しないことによって引数名を`return`しているときでもNRVOできる可能性がありますが、呼び出される側がスタック清掃を行う場合はそれができなくなります。

また、仮に呼び出す側がスタックを清掃する環境で引数名に対するNRVOが可能な場合でも、関数の引数を渡す際の配置順などはABIによって指定されているためNRVOによってその一部だけが残される場合、スタックに歯抜けが生じる可能性があります。

おそらく、これらの理由からC++標準としては引数名に対するNRVOを禁止しているのだと思われます。
