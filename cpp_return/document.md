...
自動記憶域期間というのは要するにローカルスコープのことです。`volatile`変数以外にこの対象に含まれないのは左辺値参照です。
...
つまりこの時、オーバーロード解決はオペランドの値カテゴリを調整しながら最大2回行われます。1回目のオーバーロード解決で適切なコンストラクタ（あるいは変換関数）を選択することができれば、それによってそのまま戻り値を構築することで暗黙ムーブが行われます。

### C++23 仕様の簡素化

C++20までの仕様では、`return`文において暗黙ムーブ可能なものがコピーされるときに二段階のオーバーロード解決によって可能ならムーブする、ことで暗黙ムーブをおこなっていました。このため、コピーが起こらない場合（戻り値が参照型の場合）は対象外となっており、二段階のオーバーロード解決は処理として複雑となるため正しく実装されない（振る舞いがコンパイラによって異なる）といった問題を引き起こしていました。

C++23ではそれらの問題の解決のために、この仕様がかなり単純化されました。

C++23では、暗黙ムーブ可能なもの（*implicitly movable entity*）はC++20と同じコンテキストで条件を満たしid式によって指名される場合、そのid式はムーブする資格がある（*move-eligible*）とします。そして、ムーブする資格のあるid式の値カテゴリは*xvalue*であるとされます。
...
ただし、これは万能ではなく、ローカル参照を参照返ししようとするとコンパイルは通ってしまいます。
...
ムーブする資格のある式の判定では、変数名そのものか変数名を`()`で括った式が対象となります。戻り値型推論のところで説明したように、`decltype(auto)`は変数名を`()`で括った場合はその式としての値カテゴリを型情報に含めるため、変数名が左辺値ならば左辺値参照型（`T&`）が推論されます。
...
とはいえ、RVOが行われる場合のコピー省略では暗黙ムーブ対象ではない（`return`文のオペランドが初期化式になり、変数名を指定する式ではない）ため、暗黙ムーブとコピー省略が同居しうるのはNRVOが行われる場合のみです。その場合、`return`文における戻り値構築時のコンストラクタの選択は暗黙ムーブ仕様に則って行われ、実際の処理としてはNRVOによってそのコンストラクタ呼び出しが省略される形になります。
