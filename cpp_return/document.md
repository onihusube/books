...
NRVOは関数内の全ての`return`文から同じ変数（オブジェクト）を返しているときはほぼ確実に行われますが、`return`文がそれぞれ異なるものを返しているとNRVOは困難になります。このことは単純には、関数の`return`が増えるほどNRVOが行われにくくなると思っても差し支えないでしょう。

このことはNRVOが保証されていない理由でもあります。全ての`return`から同じものを返しているときはNRVOを保証しようとする提案もありますが、まだ採択には至っていません。
