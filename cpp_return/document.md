...
*prvalue*が実体化を受けず破棄されもしていない時、それはある式の部分式となっています。この時、その*prvalue*を含む式の評価に伴って*prvalue*式も評価されその結果の値が取得されます。*prvalue*を式そのものと思うなら、*prvalue*式はそれ以上伝播できないところまで行き着くとようやく評価され、その値（何かを初期化するための値）が取得されます。それはあたかも、大元の*prvalue*式でそれを囲む最も外側の*prvalue*式を置換したかのように評価されます。
...
`T{f()}`のような式は、`f()`という*prvalue*を受けて同じ型`T`の*prvalue*を生成する式です。`T{T{}}`も同様で、このような場合は*prvalue*は実体化されず*prvalue*のまま伝播し、仮に`T`のコンストラクタにいかなる副作用があったとしてもコンストラクタ呼び出しは省略されます。

`f()`の`return`文においては、まず戻り値の初期化が`return`文のオペランドの*prvalue*で行われ、戻り値そのものも*prvalue*であることから戻り値の初期化はスキップされ、`return`文のオペランドである*prvalue*が呼び出し元まで実体化されずに伝播します。

もしこれらの場合に`U{f()}`のように型が異なると、`U`の変換コンストラクタもしくは`T`変換演算子の引数で`f()`の戻り値である*prvalue*が具体的なオブジェクトに束縛されてから変換されます。そのため、それらの変換関数の呼び出しは省略されません（この時、呼ばれるコンストラクタ/変換演算子の引数が非参照なら、そこでもコピー省略は発生します。ただし変換を行う関数の呼び出しは回避できません）。
