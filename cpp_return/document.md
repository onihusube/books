...
- `expr`が変数名を指定する式（つまり変数名だけ）の場合
    - その変数の型を取得する
- `expr`がそれ以外の式の場合
    - 式（`expr`）の型と値カテゴリを取得する

式`expr`の型とはその式を最後まで実行したときに得られる結果の型であり、これは式によりますが多くの場合は推測可能だと思います。式の値カテゴリは、式の結果の値が左辺値となるか右辺値となるかを参照修飾`&`と`&&`で区別します。これも式によりますが、式の種類によって細かく規定されているので少し複雑です。ただ非直感的な事にはならないはずで、即値ではなく一時オブジェクトを生成しないような式の場合はほぼ左辺値（`&`）になります。即値（リテラル、一時オブジェクト）の場合はprvalueというカテゴリになり、これは参照修飾が付かないことで他の2つと区別されます。
...
`auto`の場合は仕組み的には関数テンプレートの実引数推論と同じ推論が行われます。そこでは、上記初期化式の`auto`をテンプレートパラメータ`T`とした1引数関数テンプレートを考え、その仮の関数テンプレートに`return`文のオペランド（`expr`）を渡した時に`T`に推論される型が`r`の型すなわち戻り値型となります。
...
`return`文にオペランドが無い場合（`return`文が無い場合も含む）、このような変数の初期化式（仮の関数に渡す式）としては`void()`が使用されます。

実際にこのような関数テンプレートで推論される型というのは非常に複雑なルールで決まり、それを正確に理解している人はおそらく世界でも稀だとおもわれます。とはいえ、ここでは1引数の関数テンプレートであることと
