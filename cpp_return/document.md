...
これはC++11以降行われるようになっており、暗黙ムーブ（*mplicit move*）と呼ばれます。暗黙ムーブは`return`文における最適化の一種ですが許可ではなく強制であり、条件を満たし可能であれば必ず行われます。
...
C++17までは関数ローカルで定義されたローカル変数（参照ではないもの）のみが対象でしたが、その後C++20・23で段階的により多くのものを`return`文で自動ムーブできるように拡張されてきました。
...
とはいえ、レジスタのサイズには限界があり（例えば`rax`レジスタは64bit）、戻り値返しに使用するレジスタは関数呼び出し前後で退避（コピー）が発生するためクラス型の場合はそのコンストラクタとデストラクタの呼び出しが必要になります。そのため、ある一定以上のサイズのクラス型及びトリビアルではないクラス型については、戻り値のための領域を別に用意しておいて専用のレジスタを通してそのアドレスをやりとりします。この別に用意される領域には通常スタック領域が使用され、これは関数を呼び出す側が用意するのが一般的です。
...
mov     rbp, rsp
sub     rsp, 16
mov     QWORD PTR [rbp-8], rdi  # 戻り値アドレス取得
mov     rax, QWORD PTR [rbp-8]  # 戻り値アドレスコピー
mov     esi, 10                 # コンストラクタ引数積み込み
mov     rdi, rax                # thisの積み込み
call    C::C(int)               # コンストラクタ呼び出し
mov     rax, QWORD PTR [rbp-8]  # 戻り値（アドレス）返し
leave
ret
```
...
mov     rbp, rsp
push    rbx
sub     rsp, 24       # 戻り値（cの）領域確保
lea     rax, [rbp-20] # 戻り値アドレス取得
mov     rdi, rax      # 戻り値アドレス積み込み
call    f()           # 関数呼び出し
mov     ebx, 0
lea     rax, [rbp-20] # cのアドレス取得
mov     rdi, rax      # thisの積み込み
call    C::~C()       # デストラクタ呼び出し
mov     eax, ebx
...
x64 linux環境では、非トリビアルクラス型の戻り値の領域のアドレスを渡すのに`rdi`レジスタが使用されます。また、`rax`レジスタには関数リターン時に`rdi`のアドレスがコピーされます。`rdi`レジスタは厳密には関数の最初の引数を渡すのに使用されるレジスタであり、ABIの規定では`this`は関数の最初の引数であるかのように扱うとされているため`this`パラメータも`rdi`レジスタで渡されます。つまりは、戻り値のアドレスとか`this`パラメータは一貫して関数の暗黙の最初の引数として扱われています。

この例ではコンストラクタ定義を省略しているため`C`がどのようにオブジェクトを初期化するのかは不定ですが、`f()`内での`C`のコンストラクタ呼び出し時に`this`引数として`rdi`のアドレスが渡されていることから、戻り値は`f()`のローカル（スタック領域）ではなく呼び出し側`main()`の用意したスタック領域に構築されていることがわかります。

また、ここではコピー省略が行われており、`main()`で`rdi`に積んでいる戻り値専用領域とその後`c`の使用時（破棄のため）にアクセスしている場所は同じ（`[rbp-20]`）になっています。

これはつまり関数の暗黙の引数として受け側の変数の参照（ポインタ）を渡しているのと同じことで、関数内では戻り値をその領域に直接構築しています。この振る舞いはまさに、前で説明したNRVOの動作イメージと同じです。

このコードをC++11モードかつコピー省略をオフにするオプションを指定してコンパイルすると、`f()`の出力は変化しませんが呼び出し側（`main()`）の出力は変化します
...
# f()の呼び出しまでは同じなので省略
call    f()             # 関数呼び出し
lea     rdx, [rbp-20]   # 戻り値アドレス取得
lea     rax, [rbp-24]   # ローカル変数cのアドレス取得
mov     rsi, rdx        # 戻り値のアドレスを積み込み
mov     rdi, rax        # thisの積み込み
call    C::C(C const&)  # c初期化（戻り値からのコピー）
lea     rax, [rbp-20]   # 戻り値アドレス取得
mov     rdi, rax        # thisの積み込み
call    C::~C()         # 戻り値破棄
mov     ebx, 0
...
`rsi`レジスタは2つ目の引数を渡すのに`rdi`の次に通常使用されるレジスタです。4行目で`rsi`に戻り値のアドレスを入れているのは、その後6行目のコピーコンストラクタの引数として使用するためです（`rdi`には`c`の`this`引数が渡されています）。

`f()`の戻り値とローカル変数`c`が別々に扱われるようになり、戻り値からの`c`初期化（コピーコンストラクト）->戻り値の破棄（デストラクタ呼び出し）->ローカル変数破棄のように処理が行われていることがわかります。このように、アセンブリ出力からも戻り値の初期化とそれを受ける変数の初期化が別になることが確認できます。

このとき重要なのは、コピー省略を無効化したときでも戻り値返しに関して関数側がやることは変わらず、呼び出し側から渡された戻り値専用の領域に戻り値を構築してリターンしているという点です。戻り値用領域が何を指しているのかや戻り値のコピーが起こるかどうかなどのことは関数リターン後の呼び出し側で起こることで、関数はコピー省略が起こるかどうかに関わらず、デフォルトの動作としてNRVO的な戻り値の返し方をしています。ABIが指定する戻り値の返し方はコピー省略の種類によらず変わらないため、このことはRVO/NRVOが行われるかによらず同様です。

少なくとも、現在の64bit CPU環境においての主要なC++ ABI（System V AMD64 ABI、ARM64 C++ ABI、Windows x64/ARM64 ABI）では、使用するレジスタや細かい条件等の違いはあれど非トリビアルクラス型を戻り値で返す際に行われることは共通しています。

つまり、ABIから見ると関数戻り値におけるコピー省略とはデフォルトの動作であって、わざわざそこに戻り値と受け側変数の区別やコピーコンストラクタやデストラクタ呼び出しといった余計なことを加えているのはC++の都合です。コピー省略とは、C++のオブジェクト意味論のために非効率となっていた関数戻り値返しの扱いをデフォルトの状態に近づけようとするものだといえます。

### NRVOと呼び出し規約

関数の戻り値返しがABIでどのように行われているかを見てみると、NRVOの制約の理由が見えてきます。

まず、NRVOが困難となる場合に関してです。
...
この場合、`if`の`false`の分岐ではRVOが行われますが、`true`の分岐ではNRVOが行われない可能性があります。これは少し前で、`return`文で異なるオブジェクトを返しているとNRVOが行われづらくなる、と雑に説明していました。

ABIのレベルでは、呼び出し側が用意した領域のアドレスを受け取ってその領域に対して戻り値を構築するわけで、この例でNRVOが行われるとすると`c`を直接その領域に構築することになります。すると、その場合に`b`が`false`だと別のオブジェクトをその領域に上書きしなければならないため、`c`を一旦破棄してから`{20}`で再構築することになります。当然、これはC++コード上の動作とは異なります（デストラクタの呼び出しタイミングが違う）。つまりは、`return`文それぞれで異なるオブジェクトを返していると、渡された戻り値用の領域にどのオブジェクトを構築すればいいのかわからなくなるため、NRVOが行われづらくなります。

戻り値のためにどのオブジェクトを構築すべきかがわからなくなるのは本質的には分岐のせいなので、戻り値で返すオブジェクトを分岐の内側で構築するようにすると多少NRVOに優しくなります。
...
例えばx64 clang 16.0.0（`-O2`オプションを付加、Compiler Explorerを使用）は、書き換え前の`f()`はNRVOできませんが、書き換え後の`f2()`はNRVOを適用してコンパイルしてくれます
...
mov     eax, 10
mov     esi, 20
cmovne  esi, eax  # bに応じてコンストラクタ引数を調整
call    C::C(int) # rdiの領域に構築
mov     rax, rbx
pop     rbx
ret
```

次に、`return`文のオペランドの変数名がその関数の仮引数名ではないこと、というC++標準で許可されているNRVOの条件の一つに関してです。

関数の引数をどのように渡すかということもABI（呼び出し規約）の一部であり、整数型等組み込み型やトリビアルなクラス型などはレジスタに配置されて渡されます。一定サイズを超えるか非ヒトリビアルなクラス型になると、スタックに配置されそのポインタが渡され、引数が多いと後ろの方の引数は特定のスタック上に配置され渡されます。この時、このスタックを掃除（デストラクタ呼び出し等を）する責任を持つのが呼び出し側なのか呼び出された関数側なのかがABIによって異なっています。x64の主要なABIは全て呼び出し側がスタック清掃を行いますが、x86やIA-32のABIでは呼び出された側がそれを行うものもあり混在しています。

関数を呼び出す側がスタック清掃の責任を持つ場合、そのスタックを清掃しないことによって引数名を`return`しているときでもNRVOできる可能性がありますが、呼び出される側がスタック清掃を行う場合はそれができなくなります。また、仮に呼び出す側がスタックを清掃する環境で引数名に対するNRVOが可能な場合でも、関数の引数を渡す際の配置順などはABIによって指定されているためNRVOによってその一部だけが残される場合、スタックに歯抜けが生じる可能性があります。

おそらく、これらABIにおける実装面の理由から、C++標準としては引数名に対するNRVOを禁止しているのだと思われます。
