...
また、ここではコピー省略が行われており、`rdi`に積んでいる戻り値専用領域とその後`c`の使用時（破棄のため）にアクセスしている場所は同じ（`[rbp-20]`）になっています。

これはつまり関数の暗黙の引数として受け側の変数の参照（ポインタ）を渡しているのと同じことで、関数内では戻り値をその領域に直接構築しています。この振る舞い、どこかで聞いた気がしませんか？そう、NRVOがまさにこのようなことをしていました。
