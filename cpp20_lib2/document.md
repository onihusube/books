...
この静かで意図しない変更の影響を低減するために、出力ストリーム（`std::basic_ostream`）のストリーム出力演算子`<<`のうち、`char8_t`文字/文字列をとるオーバーロードは`delete`として定義されるようになります。
...
元からそうだったとはいえ、これによってユニコード文字型の文字/文字列を出力する方法はC++標準ライブラリには用意されなくなります。`std::basic_string`や`std::basic_string_view`の出力も内部でこれらのオーバーロードを使用することから、出力の可否は各文字型のリテラルと同様です。
