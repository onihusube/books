...
また、デフォルトコンストラクタの効果は以前は未規定でしたが、C++20からフラグをクリア状態にすると規定されるようになります。これによって、`ATOMIC_FLAG_INIT`マクロを使用する必要がなくなります。
...
`.test()`は引数としてメモリオーダーを指定でき、デフォルトでは`std::memory_order::seq_cst`が指定されています。
