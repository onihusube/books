...
`dst`には`char8_t`に変換した結果が書き込まれますが、ユニコードの任意の1文字はUTF-8で最大4バイトになり`char8_t`1つに収まらないため、少なくとも4要素の配列を渡すのが安全です。
...
この関数の入力`src`は`char8_t`へのポインタではなく`char8_t`を1つだけ取ります。UTF-8エンコーディングは最大4バイト（`char8_t`4つ分）になり、UTF-8エンコーディングで1バイトになる文字とはすなわちASCII範囲内の文字です。1バイトのUTF-8ははASCIIと互換性があるため、実質的にこの関数を使用する意味はなさそうに思えます（キャストで十分です）。
...
戻り値を見て、`char8_t`1つがASCII範囲内の文字かどうかを判定するのには使えるかもしれません。
