...
コンセプトによってテンプレートパラメータの制約のために結果的に`&& ||`が混在しているものの、別の制約との間で順序をつけることを意図しない場合はよくあると思われます。この場合に`&& ||`の制約を`requires`式内部に移動することによって、順序の表現を捨てる代わりにコンパイル不可の軽減を図ることができます。
...
この2つのコンセプトは制約としては同じ物を表現していますが、2つめのほうが相対的にコンパイルへの負荷が軽くなります。これは`requires`式はこれ1つで1つの原子制約式となることから、制約の順序付けの際に考慮すべき原子制約式を減少させ、標準形への変換や比較の負荷を軽減できるためです。
