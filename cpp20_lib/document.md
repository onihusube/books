...
`<chrono>`関連の値はほぼストリーム出力が可能となっているとともに、`std::format()`による文字列化もサポートされています。これは、`<chrono>`関連の型に対して`std::formatter`特殊化をあらかじめ用意する形でサポートされています。そのフォーマット構文は組み込み型のものをベースに制限と拡張を加えたもので、1つの置換フィールド全体は次のようになっています
