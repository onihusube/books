...
int main() {
  ...
  sys_seconds st;
  strm >> parse("%EY年%B%Od日 %a %OH時%OM分%OS秒"s, st);
  ...
  strm >> parse("%Y年%B%d日 %a %p %OI時%M分%S秒"s, st);
...
ロケール依存の表現については、どのロケールでどういう文字列を受理するのかが非常に不透明であるため、何を意図しているのかとかパースできる例などを示すことができませんでした。`%A %B %p`以外は有用性がよくわかりません・・・
