...
なお、この2つの関数も入力は符号なし整数型オンリーであるため、符号付き整数型において符号ビットも含めた循環シフトを行うことはできません。どうしてもそれを行いたい場合は、`std::bitcast()`によって符号付き整数型から符号なし整数型へビット表現を保った上で変換し、`std::rotl()/std::rotr()`に通した結果を再度`std::bitcast()`によって符号付き整数型へ変換する、のような手順を踏めば行うことができます。
