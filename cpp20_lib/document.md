...
これは1つの置換フィールド（`{...}`）を表しています。スペース区切りの各単語はオプション種類（グループ）を表すものであり指定する値そのものではなく、実際の指定時はスペースは入りません。また、`:`は*index*以外のオプションを指定する際に必須の文字であり、`.`は*precision*を指定する際に必須の文字です。`alt(#)`と`pad(0)`は指定可能な文字がそれぞれ`#`と`0`しかないことを表しています。
