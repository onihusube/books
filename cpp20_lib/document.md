...
文字列で表されている日付時刻をストリームからパースすることができます。パースに当たっては、`parse()`というマニピュレータにフォーマット指定と解析後に保存する`time_point`をわたして、それに対して`>>`します。
...
ここではサンプルコード表示のために`std::stringstream`を使用していますが、`std::cin`のほか任意の`istream`を同じように使用することができます。

解析のためのフォーマット指定は先程の`std::format`による出力時フォーマット指定と同様の構文であり、意味もある程度対になっています。
