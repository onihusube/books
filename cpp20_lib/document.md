...
時間を扱う上で、日付と時刻に続いて重要な要素がタイムゾーンです。タイムゾーンは、ある地域のローカル時間がUTCを基準として何時間進んでいる/遅れているかを表すものです。例えば、日本標準時はUTC+9のタイムゾーンに属しています。

ログ出力などに使用する分にはUTCやシステム時刻で事足りますが、何かしら時刻を扱うアプリケーションを作成する場合は地域ごとのローカル時間を使用するシーンが多くあるでしょう。その場合、UTC時刻とタイムゾーン情報によって地域ごとのローカル時間を計算することができ、C++20 `<chrono>`ライブラリにおいてもタイムゾーンによるローカル時間の取り扱いがサポートされています。

現在の（そのシステムローカルの）タイムゾーン情報を取得するには、`current_zone()`を使用します。この戻り値型は`time_zone`オブジェクトへのポインタです。
...
`time_zone`オブジェクトでは、`.name()`によってそのタイムゾーンの名前文字列（*地域/地名*の形式）を取得でき、`.to_local()`によってシステム時刻からそのタイムゾーンのローカル時刻へ変換することができます。
