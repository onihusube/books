...
注意点として、これら月の定数と年を結合する際には年の値に`y`リテラルが必須です。これは`mm/dd`だけでなく`dd/mm`の形式もサポートするためで、年との混同を避けるために`dd/mm`の場合`dd`に対応する値にも`d`リテラルが必要です。
...
月のカレンダー定数に対しての`[n]`は「ある月の`n`回目のその曜日」を表す`weekday_indexed`型の値を返します。あるいは、`weekday_indexed`型のコンストラクタに曜日定数と`n`を渡して構築することもできます。これはまだ特定の日付を表す値ではなく、月および年と結合してもまだ日付にはなりません。

月及び年と結合したうえで`sys_days`（日単位のシステム時刻）に変換することで日付を取得できます。なお、この変換は暗黙変換が用意されています。
...
ここまで紹介してきたカレンダー型とカレンダー定数による日付の表現について、その結果の値にのみ着目して型については触れないできました。これらの値は、その生成のされ方によって細かく型付けされており、それによって結合できる値を制限したり、時刻への変換可能性を制御したりしています。

例えば、`y, d`リテラルやカレンダー定数から生成される、単一の期間単位を表す値は次のような型によって表現されています。
...
そして、これらの値及び整数値の間で`/`によって結合したときも、その結合の仕方（引数型と順序）によって細かく異なる型が生成されています。

|型名|意味|生成される例|
|---|---|---|
|`month_day`|月日を表す型|`September/11d`|
|`month_weekday`|月と`n`回目の曜日を表す型|`October/Monday[2]`|
|`year_month`|年月を表す型|`2021y/11`|
|`year_month_day`|年月日を表す型|`1181y/3/27`|
|`year_month_weekday`|年月と`n`回目の曜日を表す型|`2022y/October/Monday[2]`|

1度の`/`の結合だけでは完全な日付とはなっておらず、`year_month`型などの中間表現の値が生成されています。適切に2回`/`で結合することで`year_month_day`のような完全な日付を表す値になります。

このように細かく型付けを行うことによって、日付の表現と構文との対応を厳密に管理するとともに、日付形式に沿った結合の制御を行っています。

### 日付形式と`/`

`/`ではかなり柔軟な値の結合をサポートしているように見えますが無秩序ではなく、基本的な3種類の日付形式に沿った結合しかサポートしていません。
...
`yyyy`に指定する値は`y`リテラルによる値、`mm`に指定する値は月を表すカレンダー定数ですが、`dd`に指定できる値には`d`リテラル

また、結合に使用する`/`は言語組み込みの演算子ではなく演算子オーバーロードによるものです。従って、`/`は通常と同じく左結合であり、左側と真ん中の引数(年月日なら`yyyy, mm`)の両方が単なる整数値（`int`型）だと使用される`/`は整数演算を行ってしまうため、カレンダーを表現するものではなくなってしまいます。これは、年（`y`リテラルによる値）が右側に来る形式（月日年と日月年）だと、年の値の左辺に整数値が来ないことがわかるためコンパイルエラーとなりますが、年月日形式では日の値（`dd`）は月日
