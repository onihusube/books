...
2のべき乗値への切り上げ/切り捨てとはすなわち、その値に最も近い2のべき乗値へ値を丸めることです。丸め方にはその値より大きいか小さいかで二通りあり、それぞれ切り上げ切り捨てに対応しています。別の言い方をすると、2のべき乗値への床関数/天井関数です。

切り上げには`std::bit_ceil()`を、切り捨てには`std::bit_floor()`を使用します。
