...
`last`定数を用いると、2月の最終日やある月の最後の日曜日などの日単位の最後（上限）が必ずしもはっきりしていない日付について、それらの文脈に応じた最後の日という形で指定することができます。
...
この`last`という値は日単位を表す値であるので、`d`リテラルの値と同じように結合を行うことができます。
...
そして、これらの値及び整数値の間で`/`によって結合したときも、その結合の仕方（引数型）によって細かく異なる型が生成されます。
...
カレンダー型とその操作はほぼ`constexpr`対応されているため定数式で使用可能です。また、動的な日付を表現したい場合はこれらカレンダー型（特に`year, month, day`型）のコンストラクタに月や日を表す整数値を渡して構築することで実行時の値として取得でき、以降は`/`を同じように使用することができます。
