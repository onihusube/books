...
パースのためのフォーマット指定は先程の`std::format`による出力時フォーマット指定と同様の構文であり、意味もある程度対応しています。
...
`%y`は年の末尾2桁で年を読み取るもので、デフォルトだと`[69, 99]`が`[1969, 1999]`年に、`[00, 68]`が`[2000, 2068]`年にマッピングされます。この時、`%C`によって世紀を指定するとそのマッピングを変更することができます。
...
この基本的なフォーマット指定だけを見ていてもわかりますが、大文字と小文字の間違いは大きな意味の違いを招きますので気をつけましょう。
...
int main() {
  ...
  strm >> parse("%FT%T %Z %z"s, st, tz_name, offset);
...
年や日、時分などの基本単位を指定するフォーマット指定では、頭に`O`を付加することでロケール依存フォーマット指定となり、`E`を付加するとロケール依存の代替表現フォーマット指定になります。

|フォーマット指定|意味|例（日本語ロケール）|
|---|---|---|
|`%EC`, `%OC`|元号|`令和`|
|`%EY`|年の代替表現|`令和元年`(=2019年)|
|`%Oy`|年の後ろ2桁|`99年`(=1999年)|
|`%Ey`|年の後ろ2桁の代替表現|`03年`(=令和3年=2021年)|
|`%Om`|月|`12月`|
|`%Od`, `%Oe`|日|`6日`|
|`%OH`|時（24時間時計）|`22時`|
|`%OI`|時（12時間時計）|`9時`|
...
これらロケール依存フォーマット指定においては、対応するロケール非依存フォーマット指定（`O`/`E`を取り除いたもの）が存在しているものについてはそちらも同時に考慮されます。
...
int main() {
  ...
  std::stringstream strm{};
  strm.imbue(loc);
  //strm << "2022年6月21日 火 11時11分11秒";
  strm << "2022年6月21日";

  year_month_day ymd;

  strm >> parse("%Y年%B%d日", ymd);
...
|複合フォーマット|等価な指定|受理する例|
|---|---|---|
|`%c`|`%Y年%Om%Od [%a ]%OH%OM%OS`|`2022年6月21日 火曜日 11時11分11秒`|
|`%Ec`|`%EY%Om%Od [%a ]%OH%OM%OS`|`令和4年6月21日 火曜日 11時11分11秒`|
|`%r`|`%p%OI%OM%OS`|`午後 1時25分59秒`|
|`%x`|`%Y年%Om%Od`|`2022年6月21日`|
|`%X`|`%OH%OM%OS`|`11時11分11秒`|
|`%Ex`|`%EY%Om%Od`|`令和4年6月21日`|
|`%EX`|`%OH%OM%OS`?|??|

表中の等価な指定では、`[%a]`のような表記はそれが省略可能であることを表しています。
...
|`%OW`|ロケール依存週番号による年※1||
|`%j`|日単位の年指定|`364`|
|`%Nj`|`N`桁の日単位の年指定|`00364`|

※1 1年の最初の月曜日が最初の曜日であるとする週番号。`%U`は年の最初の日曜日を最初の曜日とする
...
- 日付と時刻の表示形式 (LC_TIME カテゴリ)  
  (https://docs.oracle.com/cd/E19683-01/816-3899/x-4101g/index.html)
