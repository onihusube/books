...
拙著「C++20 ライブラリ機能1」では簡易的ながらもこれと同等のクラスを例として作成していましたが、それと同等かつより洗練されたものが標準ライブラリ機能として利用できるようになります。この例のように、`std::generator`はそれそのものが`range`（常に`input_range`）となるため、範囲`for`文や`<ranges>`の機能、Rangeアルゴリズムなどと容易に組み合わせて使用できます
