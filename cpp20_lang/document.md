...
int main() {
  // この二つの初期化構文は同じ意味
  point3d vec1 = { 2.0, 3.0, 1.0 };
  point3d vec2 = { .X = 2.0, .Y = 3.0, .Z = 1.0 }; // C++20から
}
```
...
int main() {
  ...
  nest n1{ .n = 1, .p.X = 1.0, .p.Y = 1.0, .p.Z = 1.0, .m = 2 };    // ng
...
このような指示付初期化は初期化するメンバ名を明確にすることによって初期化指定の間違いを発見しやすくする効果があり（間違えるとコンパイルエラーになる）、大量のメンバを持つ構造体の初期化を読みやすく書くことができます。また、共用体の初期化時には、初期化するメンバを指定して初期化することができます。
