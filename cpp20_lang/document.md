...
集成体と同様に、しかし異なる理由からあるクラステンプレートの初期化時に。エイリアステンプレートを経由するとCTADは無効になっていました。
...
ただし、エイリアステンプレートに対して推論補助を書く事ができるようにはなっていません。あくまで、エイリアス元のクラスの推論補助やコンストラクタを利用して推論を行えるようになっただけです。

この手順は少し複雑ですが、概ね次のように実行されています

1. エイリアステンプレートの右辺と元の型の対応する推論補助の右辺から、テンプレートパラメータの対応を求める
2. その対応から元の推論補助にエイリアステンプレートのパラメータをフィードバックして置き換える
3. 得られた推論補助の右辺と左辺を元のエイリアスの右辺と左辺で置き換えることで、エイリアステンプレートの推論補助を生成する
4. その推論補助を用いて通常の手順でCTADを行う

この概略では推論補助の存在を求めていますが、実際には推論補助は必要ありません。その場合は実引数とエイリアスに対応するコンストラクタから推論補助相当のものが補われて上記手順が実行されます。
