...
C++17までは`this`の明示的なキャプチャは許可されておらず、`[=]`か`[&]`によって暗黙にキャプチャする事しかできませんでした。C++17で`this`の値キャプチャ（`*this`のキャプチャ）が許可されたことで、`this`を明示的にキャプチャする事が望ましい場合があります。
...
std::function<void()> X::f(int n) {
  return coin() ? [=]() { g(n, x_, y_, z_); }         // thisの暗黙キャプチャ
                : [=, *this]() { h(n, x_, y_, z_); }; // thisの値キャプチャ
}
```

このように、`[=]`と`[=, *this]`の両方がコード中にいくつか存在している場合、その2つの構文の違いが忘れられがちになるため、キャプチャの種類を明示することで可読性を向上させたい事があります。しかし、`[=]`によるキャプチャは`this`を暗黙にコピーキャプチャしているため、重複して指定することは禁止されていました。

C++20より、可読性の向上のために`[=, this]`のように明示的な`this`のコピーキャプチャが許可されます。

```cpp
std::function<void()> X::f(int n) {
  return coin() ? [=,  this]() { g(n, x_, y_, z_); }  // thisの明示キャプチャ
                : [=, *this]() { h(n, x_, y_, z_); }; // thisの値キャプチャ
}
...
そして逆に`[=]`による`this`の暗黙キャプチャは非推奨とされ、`this`をコピーキャプチャする場合は常に`[=, this]`と書く事が推奨されるようになります。これは上記の可読性の問題に加えて、あるクラススコープで`[=]`によってコピーキャプチャをしているときに、そのクラスのメンバはコピーキャプチャではなく暗黙キャプチャされた`this`を介した参照キャプチャになるという構文と意味の非一貫性が理由です。
