...
クラス型に対して`<=>`を定義しておくと、`< <= > >=`の4つの演算子が自動で実装され、それが`dafault`の場合は`== !=`の2つも自動で実装されます。これらの自動で実装される比較演算子は`<=>`を利用して実装されています。`== !=`だけ扱いが異なるのは、この2つは`==`演算子を用いて実装されていて、`<=>`が`default`で宣言されている場合は暗黙的に`==`も`default`定義されているためです。
...
自動実装に使用されている`default`の`<=> ==`もまた自動実装されており、最小のコードで比較演算子自動実装が可能となっています。この`default`の`<=> ==`定義をあえて明示的に書いてみると、次のようになります
...
こうしてみてみると、`int`等の基本型に`<=>`が用意されているのは、この`default`実装のためであることが分かります。`<=>`は普段使いする演算子というよりは、比較演算子自動実装のためのマーカーのようなものとしての側面が強いです。
...
- 全順序 : すべての値がお互いに比較可能
- 弱順序 : 一部の値の間で比較不可能なペアが存在する
- 半順序 : 他の全ての値と比較不可能な値が存在する
...
それはC++17まではその通りだったのですが、ハッシュ値の簡易な計算のためやこの`<=>`のためなどの理由と、実際には2の補数以外の実装が無かったことから、C++20からは符号付整数型の内部表現は2の補数であると規定されるようになります。
