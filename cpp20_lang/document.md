...
コンストラクタ引数からのテンプレートパラメータ推論（CTAD）ではファクトリ関数を省略できクラス名をさも関数であるかのように書くことができるようになるため、変数を定義するのではなくコンストラクタを呼び出しただけの様な間違いが起こりやすくなることが想定されるため、コンストラクタに対する`[[nodiscard]]`はC++17以降特に重要であり、それを明示的に許可することになりました。
